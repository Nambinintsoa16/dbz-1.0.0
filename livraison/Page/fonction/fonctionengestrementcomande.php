...
if(isset($_POST['date']) AND isset($_POST['Debut']) AND isset($_POST['Fin']) AND isset($_POST['ville']) AND isset($_POST['quartier']) AND isset($_POST['lieulivre']) AND isset($_POST['remarque']) AND isset($_POST['quantiter']) AND isset($_POST['produitr']) AND isset($_POST['fact']) AND isset($_POST['datecommande']) AND isset($_POST['client'])){
    ...
    if($reponse){
$sqllivre="INSERT INTO `gestiondevente`.`livraison` (`lieudelivraison`, `datedelivraison`, `debut`, `fin`,`ville`,`qartieur`, `idcomand`) VALUES ('".$_POST['lieulivre']."','".$_POST['date']."','".$_POST['Debut']."','".$_POST['Fin']."','".$_POST['ville']."','".$_POST['quartier']."','".$reponse['idcomand']."')";
    $main->fetch($sqllivre);
    $dt = new DateTime();
    date_default_timezone_set('UTC');
    $Date=$dt->format('Y-m-d');
    $sqlBl='INSERT INTO `gestiondevente`.`facture` (`idfacture`, `idcomande`, `datedefacture`, `NumFact`, `user`,`idclient`) VALUES (NULL, "'.$reponse['idcomand'].'", "'.$Date.'", "'.$_POST['fact'].'", "'.$_SESSION['login']['matricule'].'","'.$client[0].'")';
    $main->fetch($sqlBl);
    $dt->modify("+45 day");
    $date=$dt->format("Y-m-d");
